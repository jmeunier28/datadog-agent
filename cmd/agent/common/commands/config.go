...
func Config(getClient settings.ClientFunc) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "config",
...
func listRuntime(getClient settings.ClientFunc) *cobra.Command {
...
func set(getClient settings.ClientFunc) *cobra.Command {
	return &cobra.Command{
...
func get(getClient settings.ClientFunc) *cobra.Command {
	return &cobra.Command{
		Use:   "get [setting]",
...
func showRuntimeConfiguration(getClient settings.ClientFunc) error {
	c, err := getClient()
	if err != nil {
		return err
	}
...
func listRuntimeConfigurableValue(getClient settings.ClientFunc) error {
	c, err := getClient()
	if err != nil {
		return err
	}
...
func setConfigValue(getClient settings.ClientFunc, args []string) error {
	if len(args) != 2 {
...
func getConfigValue(getClient settings.ClientFunc, args []string) error {
	if len(args) != 1 {
